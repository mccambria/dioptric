...
class OPX():
    ...
    def get_full_seq(self, seq_file, seq_args_string, num_repeat, x_voltages, y_voltages, z_voltages): # this one returns the full qua sequence with repeats, such as rabi with nreps=1e5. DONE
        ...
        if file_ext == ".py":  # py: import as a module
            ...
            seq, final, ret_vals = seq_module.get_full_seq(        # here seq is the qua program. refer to opx sequence template      
                 args, num_repeat, x_voltages, y_voltages, z_voltages
            )
    ...
    def stream_load(self, seq_file, seq_args_string=""): #from pulse streamer. for the opx all this function does is return back some property(s) of the sequence you want. DONE
    ...
    def stream_start(self,  num_repeat=1): # from pulse streamer. It will start the full sequence. DONE
    ...
    def stream_immediate(self, seq_file, num_repeat=1, seq_args_string=""): # from pulse streamer. For the OPX this will get the full sequence we want and run it. DONE
    ...
    def read_counter_complete(self,  num_to_read=None):
    ...
    def read_counter_simple(self, num_to_read=None):
    ...
    def read_counter_separate_gates(self, num_to_read=None):
    ...
    def read_counter_modulo_gates(self,  modulus, num_to_read=None):
    ...
    def read_tag_stream(self,  num_to_read=None): #from apd tagger ###need to update
        ...
        """
        results_counts = fetching_tool(self.experiment_job, data_list=["counts"], mode="wait_for_all")
        results_times = fetching_tool(self.experiment_job, data_list=["times"], mode="wait_for_all")
        counts = results_counts.fetch_all()
        times = results_times.fetch_all()
        
        counts = counts[0][0].tolist()
        times = times[0].tolist()
    ...
    def start_tag_stream(self, apd_indices, gate_indices=None, clock=True):
        """opx doesn't need it"""
        pass
    
    # @setting(9, apd_indices="*i", gate_indices="*i", clock="b") # from apd tagger. 
    def stop_tag_stream(self, apd_indices, gate_indices=None, clock=True):
        """OPX doesn't need it"""
        pass
    
    # @setting(10)
    def clear_buffer(self): # from apd tagger
        """OPX doesn't need it"""
        pass
