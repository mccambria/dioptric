...
class Galvo(LabradServer):
    ...
    @setting(6, x_center='v[]', y_center='v[]',
             x_range='v[]', y_range='v[]', num_steps='i', period='i',
             returns='*v[]*v[]')
    def load_sweep_scan_flip(self, c, x_center, y_center, 
                        x_range, y_range, num_steps, period):
        """Load a scan that will wind through the grid defined by the passed
        parameters. Samples are advanced by the clock. Currently x_range
        must equal y_range.
    ...
    @setting(7, x_center='v[]', y_center='v[]',
             x_range='v[]', y_range='v[]', num_steps='i', period='i',
             returns='*v[]*v[]')
    def load_sweep_scan_bl(self, c, x_center, y_center, 
                        x_range, y_range, num_steps, period):
        """Load a scan that will wind through the grid defined by the passed
        parameters. Samples are advanced by the clock. Currently x_range
        must equal y_range.
        ...
        # are linear in these voltages. For a small range, however, we don't
        # really care.
        x_voltages_1d = numpy.linspace(x_high, x_low, num_steps)
        y_voltages_1d = numpy.linspace(y_low, y_high, num_steps)
        ...
        # [1, 2, 3] => [1, 2, 3, 3, 2, 1]
        y_inter = numpy.concatenate((y_voltages_1d,
                                     numpy.flipud(y_voltages_1d)))
        # [1, 2, 3, 3, 2, 1] => [1, 2, 3, 3, 2, 1, 1, 2, 3]
        if x_num_steps % 2 == 0:  # Even x size
            y_voltages = numpy.tile(y_inter, int(x_num_steps/2))
        else:  # Odd x size
            y_voltages = numpy.tile(y_inter, int(numpy.floor(x_num_steps/2)))
            y_voltages = numpy.concatenate((y_voltages, y_voltages_1d))

        # [4, 5, 6] => [4, 4, 4, 5, 5, 5, 6, 6, 6]
        x_voltages = numpy.repeat(x_voltages_1d, y_num_steps)
    ...
    @setting(8, x_center='v[]', y_center='v[]',
             x_range='v[]', y_range='v[]', num_steps='i', period='i',
             returns='*v[]*v[]')
    def load_sweep_scan_ul(self, c, x_center, y_center, 
                        x_range, y_range, num_steps, period):
        """Load a scan that will wind through the grid defined by the passed
        parameters. Samples are advanced by the clock. Currently x_range
        must equal y_range.
        ...
        # really care.
        x_voltages_1d = numpy.linspace(x_high, x_low, num_steps)
        y_voltages_1d = numpy.linspace(y_high, y_low, num_steps)
    ...
    @setting(9, x_center='v[]', y_center='v[]',
             x_range='v[]', y_range='v[]', num_steps='i', period='i',
             returns='*v[]*v[]')
    def load_sweep_scan_ur(self, c, x_center, y_center,
                        x_range, y_range, num_steps, period):
        """Load a scan that will wind through the grid defined by the passed
        parameters. Samples are advanced by the clock. Currently x_range
        must equal y_range.
