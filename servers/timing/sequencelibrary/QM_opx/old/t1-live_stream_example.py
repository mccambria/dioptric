...
t_vec = np.array([4,8,12,16])  # +0.1 to include t_max in array
n_avg = 150000
...
with program() as T1:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(t, t_vec)):
            # if start_from_one:
                # play("pi", "NV")
            # wait(t, "NV2")  # variable delay in spin Echo

            align()

            # play("laser_ON", "AOM")
            measure("readout", "APD_0", None, time_tagging.analog(times1, meas_len, counts1))
            ...
            # play("laser_ON", "AOM")
            measure("readout", "APD_0", None, time_tagging.analog(times2, meas_len, counts2))
...
if simulate:
    ...
    job = qmm.simulate(config_opx, T1, simulation_config)
