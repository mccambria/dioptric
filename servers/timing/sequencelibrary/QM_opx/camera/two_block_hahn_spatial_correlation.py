...
def get_seq(base_scc_seq_args, tau, T_lag, num_reps=1):
    tau_cc = seq_utils.convert_ns_to_cc(tau)
    T_lag_cc = seq_utils.convert_ns_to_cc(T_lag)
    with qua.program() as seq:
        seq_utils.init()
        seq_utils.macro_run_aods()

        def uwave_macro_sig(uwave_ind_list, step_val):
            # entry π/2 (x + phase cycle)
            seq_utils.macro_pi_on_2_pulse(uwave_ind_list, phase=0)
            qua.wait(tau_cc)
            # π pulse (same axis)
            seq_utils.macro_pi_pulse(uwave_ind_list, phase=0)
            qua.wait(tau_cc)
            # exit π/2: +90° for phase-quadrature (linear response), else in-phase
