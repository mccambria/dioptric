...
def get_seq(pulse_streamer, config, args):
    ...
    iq_trigger_time = min(pi_on_2_pulse/2, 10)
    ...
    rep_train = [(tau_shrt, LOW), (iq_trigger_time, HIGH), 
                 (pi_pulse-iq_trigger_time, LOW), (tau_shrt, LOW)]*pi_pulse_reps
    ...
    uwave_iq_train_long = [(iq_trigger_time, HIGH), 
                           (pi_on_2_pulse-iq_trigger_time, LOW)]
    rep_train = [(tau_long, LOW), (iq_trigger_time, HIGH), 
                 (pi_pulse-iq_trigger_time, LOW), (tau_long, LOW)]*pi_pulse_reps
    ...
    train = [(delay_buffer - iq_delay_time, LOW),
              (polarization_time,LOW),
              (pre_uwave_exp_wait_time - iq_forward_shift_shrt, LOW)]
    train.extend(uwave_iq_train_shrt)
    train.extend([
              (iq_forward_shift_shrt, LOW),
              (post_uwave_exp_wait_time, LOW),
              (signal_time, LOW),
              (sig_to_ref_wait_time_shrt, LOW),
              (reference_time, LOW),
              (pre_uwave_exp_wait_time - iq_forward_shift_long, LOW)])
    train.extend(uwave_iq_train_long)
    train.extend([
              (iq_forward_shift_long, LOW),
              (post_uwave_exp_wait_time, LOW),
              (signal_time, LOW),
              (sig_to_ref_wait_time_long, LOW),
              (reference_time, LOW),
...
if __name__ == '__main__':
    ...
    seq_args = [10, 1000.0, 100, 50, 25, 500, 8*2, 1, 3, 'integrated_520', None]
