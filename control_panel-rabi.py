...
from utils.constants import NVSig, NVSpinState, VirtualLaser
...
def do_widefield_image_sample(nv_sig, num_reps=1):
    nv_sig[VirtualLaser.IMAGING] = yellow_laser_dict
...
def do_scanning_image_sample(nv_sig):
    scan_range = 4
    num_steps = 60
    nv_sig[VirtualLaser.IMAGING] = green_laser_dict
...
def do_optimize_green(nv_sig, do_plot=True):
    coords_suffix = tb.get_laser_name(VirtualLaser.IMAGING)
...
def do_optimize_red(nv_sig, do_plot=True):
    laser_key = VirtualLaser.IONIZATION
...
def do_optimize_loop(nv_list, coords_suffix, scanning_from_pixel=False):
    ...
    for nv in nv_list:
        ...
        if coords_suffix is None:
            imaging_laser = tb.get_laser_name(VirtualLaser.IMAGING)
