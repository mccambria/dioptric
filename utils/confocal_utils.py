...
def threshold_counts(nv_sig, sig_counts, ref_counts=None, dynamic_thresh=False):
    ...
    if dynamic_thresh:
        ...
        if ref_counts is not None:
            combined_counts = np.append(sig_counts.flatten(), ref_counts.flatten())
        else:
            combined_counts = sig_counts.flatten()
...
def process_counts(nv_sig, sig_counts, ref_counts=None, readout=None, threshold=False):
    _validate_counts_structure(sig_counts)
    _validate_counts_structure(ref_counts)
    ...
    if threshold:
        if ref_counts is not None:
            sig_counts, ref_counts = threshold_counts(
                nv_sig, sig_counts, ref_counts, dynamic_thresh=True
            )
        else:
            sig_counts = threshold_counts(nv_sig, sig_counts, dynamic_thresh=True)
    ...
    if readout is not None:
        ...
        avg_counts_kcps = avg_counts / (readout_sec * 1000)
        avg_counts_ste_kcps = avg_counts_ste / (readout_sec * 1000)
        return (norm_counts, norm_counts_ste, avg_counts_kcps, avg_counts_ste_kcps)
    else:
...
def get_base_scc_seq_args(nv_list: list[NVSig], uwave_ind_list: list[int]):
    ...
    scc_duration, scc_amp = get_pulse_parameter_lists(nv_list, VirtualLaserKey.SCC)
...
def get_pulse_parameter_lists(nv_sig: NVSig, virtual_laser_key: VirtualLaserKey):
    duration_list = []
    amp_list = []
    duration = nv_sig.pulse_durations.get(virtual_laser_key)
    amp = nv_sig.pulse_amps.get(virtual_laser_key)
