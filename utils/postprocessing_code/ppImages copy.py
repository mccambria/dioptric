...
def post_process_saved_data(file_path, bin_width=50):
    ...
    sig_img_array_list_1d = saved_data["sig_img_array"]
    ref_img_array_list_1d = saved_data["ref_img_array"]

    # Infer image shape from the flattened arrays
    num_images = int(np.sqrt(len(sig_img_array_list_1d[0])))
    # Convert 1D arrays back to 2D arrays
    img_shape = len(sig_img_array_list_1d)
    print("num images", img_shape)

    sig_img_array_list = sig_img_array_list_1d.reshape((img_shape, img_shape))
    ref_img_array_list = ref_img_array_list_1d.reshape((img_shape, img_shape))

    bg_coords_outside = (90, 90)  # Background coordinates outside aperture
    bg_coords_inside = (292, 303)  # Background coordinates inside aperture
...
# Example usage
file_path = "/Users/schand/Library/CloudStorage/GoogleDrive-schand@berkeley.edu/Shared drives/Kolkowitz Lab Group/nvdata/pc_rabi/branch_master/image_sample_diff/2023_11/2023_11_14-12_16_43-johnson-nv0_2023_11_09.npz"
