...
#    avg_norm_count_list = []
    ...
    for test_pulse_length in test_pulse_dur_list:
        ...
        sig_counts, ref_counts = main(nv_sig, apd_indices, ao_638_pwr, 
...
#        avg_norm_counts = numpy.average(norm_counts)
        ...
        avg_sig_count_list.append(avg_sig_count)
        avg_ref_count_list.append(avg_ref_count)
#        avg_norm_count_list.append(avg_norm_counts)
        
    norm_avg_count_array = numpy.array(avg_sig_count_list) / numpy.array(avg_ref_count_list)
                                    
    fig, ax = plt.subplots(1,1, figsize = (10, 8.5))    
    ax.plot(numpy.array(test_pulse_dur_list) / 10**3, avg_sig_count_list, 'go', label = 'Test pulse')
    ax.plot(numpy.array(test_pulse_dur_list) / 10**3, avg_ref_count_list, 'ko', label = 'W/out test pulse')
    ...
    ax.plot(numpy.array(test_pulse_dur_list) / 10**3, norm_avg_count_array, 'bo')
    ax.set_xlabel('Test pulse length (us)')
    ...
    raw_data = {'timestamp': timestamp,
            ...
            'num_runs': num_runs,
            'num_reps':num_reps,
            'avg_sig_count_list': avg_sig_count_list,
            'avg_sig_count_list-units': 'counts',
            'avg_ref_count_list': avg_ref_count_list,
            'avg_ref_count_list-units': 'counts',
            'norm_avg_count_array': norm_avg_count_array.tolist(),
            'norm_avg_count_array-units': 'arb'
            }
...
#    test_pulse_length = 10**4 # ns
    test_pulse_dur_list = numpy.linspace(1000, 20000, 3)
