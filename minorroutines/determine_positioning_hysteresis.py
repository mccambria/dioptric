...
def main(nv_sig, movement_displ, axis_ind, apd_indices, plot=True):
    ...
    with labrad.connect() as cxn:
        opti_delta = main_with_cxn(
            cxn, nv_sig, movement_displ, axis_ind, apd_indices, plot
        )
...
def main_with_cxn(cxn, nv_sig, movement_displ, axis_ind, apd_indices, plot):
    ...
    scan_range_list = [xy_opti_scan_range, xy_opti_scan_range, z_opti_scan_range]
    axis_units = [x_units, y_units, z_units]
    ...
    for n in range(num_steps):
        ...
        ref = optimize.stationary_count_lite(
            cxn, nv_sig, init_coords, config, apd_indices
        )
        time.sleep(1)
        ...
        sig = optimize.stationary_count_lite(
            cxn, nv_sig, return_coords, config, apd_indices
        )
    ...
    try:
        opti_params, cov_arr = curve_fit(
            tool_belt.gaussian, delta_list, count_ratio_list, p0=init_fit
        )
    except Exception as e:
        print(e)
        return None

    lin_radii = numpy.linspace(delta_list[0], delta_list[-1], 100)
