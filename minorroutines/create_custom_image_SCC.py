...
def read_img(file):
    """
    Read in the image file to convert into an array of the pixel info
    """
    
    #Opens the Image
    img_file = Image.open(file)
    
    ##Creates an arrya based on the pxel info
    img_arr = numpy.array(img_file)
    
    return img_arr, img_file
...
#    readout = 100*10**6
    ...
    img_arr, img_file = read_img(file)
    
    x_num_steps = img_file.size[0]
    y_num_steps = img_file.size[1]
    
    if x_num_steps != y_num_steps:
        raise RuntimeError('x and y dimensions must match for now.')
    else: 
        num_steps = x_num_steps 
        
    # define a pixel size in Volts. Lt's try 0.005 V/px for now.
    px_size = 0.005 #V
    
    x_range = px_size * num_steps
    y_range = x_range
    ...
    # %% First we want to scan over the whole image with green light.
    
    # Load the PulseStreamer
        
    seq_args = [delay, readout, aom_ao_589_pwr, apd_indices[0], 532]
...
#    pixel_size = x_voltages[1] - x_voltages[0]
    ...
    seq_args = [delay, readout, aom_ao_589_pwr, apd_indices[0], 589]
    seq_args_string = tool_belt.encode_seq_args(seq_args)
    ret_vals = cxn.pulse_streamer.stream_load('simple_readout.py',
                                              seq_args_string)
    period = ret_vals[0]
