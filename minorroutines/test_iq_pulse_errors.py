...
def measurement(cxn, 
    ...
    for n in range(num_runs):
        ...
        ref_0_ste_run = numpy.std(
            ref_0_counts, ddof=1
            ) / numpy.sqrt(num_reps)
        ref_H_ste_run = numpy.std(
            ref_H_counts, ddof=1
            ) / numpy.sqrt(num_reps)
        sig_ste_run = numpy.std(
            sig_counts, ddof=1
            ) / numpy.sqrt(num_reps)
...
def replot_change_freq(file, folder):
    ...
    plot_errors_vs_changed_param(d_freq * 1e3,
                                 'Detuning (MHz)',
                           phi_p_list,
                           chi_p_list,
                           phi_list,
