...
def build_voltages(start_coords_drift, end_coords_drift, num_steps):
    num_steps = 121
...
# %%
def moving_target(nv_sig, start_coords, end_point, num_steps, num_runs, init_color, pulse_color):
    with labrad.connect() as cxn:
        moving_target_with_cxn(cxn, nv_sig,start_coords,  end_point, num_steps,num_runs,  init_color, pulse_color)
    return
        
def moving_target_with_cxn(cxn, nv_sig,start_coords,  end_coords, num_steps,num_runs, init_color, pulse_color):
...
#    return
    ...
    for r in range(num_runs):  
        print( 'run {}'.format(r))
        # optimize on the readout NV (where should this really go if )
        ...
        # Load the galvo
        x_voltages, y_voltages = build_voltages(start_coords_drift, end_coords_drift, num_steps)
        return
