...
def main_with_cxn(cxn, delay, readout_time, apd_index, laser_name, laser_power, num_reps ):
    ...
    readout=350
    num_repeat = 3e5
    seq_args = [int(readout), state, laser_name, int(laser_power), int(apd_index)]
    # seq_args = [int(delay), int(readout_time), int(apd_index), laser_name, int(laser_power) ]
    # print(seq_args)
    seq_args_string = tool_belt.encode_seq_args(seq_args)
    
    seq_file = 'resonance_modulo_test.py'
    ...
    total_counts = counter_server.read_counter_modulo_gates(2)
