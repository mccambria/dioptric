...
def main_s(nv_sig, apd_indices, num_reps, initial_pulse_time, dark_time, init_pulse_color):
    ...
    with labrad.connect() as cxn:
        sig_counts = main_s_with_cxn(cxn, nv_sig, apd_indices, num_reps, initial_pulse_time, dark_time,  init_pulse_color)
...
# Initial Calculation and setup
    ...
    for i in range(num_reps):    
        ...
        seq_args = [init_pulse_delay, initial_pulse_time, aom_ao_589_pwr,
                    init_pulse_color]
        seq_args_string = tool_belt.encode_seq_args(seq_args)
        ...
        seq_args = [aom_589_delay, readout_time, aom_ao_589_pwr,apd_indices[0],
                    589]
        seq_args_string = tool_belt.encode_seq_args(seq_args)
        # Clear the buffer
        cxn.apd_tagger.clear_buffer()
...
#                               10**7, 5*10**7]
    ...
    for test_pulse_length in test_pulse_dur_list:
        if test_pulse_length < 5*10**8:
            ...
            sig_count = main_s(nv_sig, apd_indices, num_reps,initial_pulse_time, test_pulse_length, 638)
            sig_count_list.append(sig_count)
    # Plot
...
def do_dark_time_w_green(nv_sig, test_pulse_dur_list = None):
    ...
    for test_pulse_length in test_pulse_dur_list:
        if test_pulse_length < 5*10**8:
            ...
            print('{} s'.format(test_pulse_length/10**9))
            sig_count = main_s(nv_sig, apd_indices, num_reps,initial_pulse_time, test_pulse_length, 532)
            sig_count_list.append(sig_count)
