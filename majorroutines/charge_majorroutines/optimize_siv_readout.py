...
# %%
def plot_time_sweep(
    test_pulse_dur_list, sig_counts_avg, ref_counts_avg, snr_list, title, text=None
):
...
def compile_raw_data_length_sweep(
    nv_sig,
...
def main(nv_sig, apd_indices, num_reps, green_pulse_time, dx, readout_color):
    with labrad.connect() as cxn:
        sig_counts, ref_counts = main_with_cxn(
            cxn, nv_sig, apd_indices, num_reps, green_pulse_time, dx, readout_color
        )
...
def main_with_cxn(
    ...
    # Optimize
    opti_coords = targeting.main_with_cxn(cxn, nv_sig, apd_indices, 532, disable=False)
...
def optimize_readout_pulse_length(
    nv_sig,
    readout_color,
    test_pulse_dur_list=[
        10 * 10**3,
    ...
    for test_pulse_length in test_pulse_dur_list:
        ...
        sig_count, ref_count = main(
            nv_sig, apd_indices, num_reps, green_pulse_time, dx, readout_color
        )
    ...
    if readout_color == 638:
        ...
        text = (
            "Red readout pulse power set to " + "%.0f" % (red_optical_power_mW) + " mW"
        )

    if readout_color == 589:
        title = "Sweep pulse length for 589 nm readout, SiV band"
        text = (
            "Yellow readout pulse power set to "
            + "%.0f" % (yellow_optical_power_mW * 10**3)
            + " uW"
        )
    fig = plot_time_sweep(
        test_pulse_dur_list, sig_counts_avg, ref_counts_avg, snr_list, title, text=text
    )
