...
def main_with_cxn(
    ...
    if plot_figure:
        ...
        ax.set_xlabel(
            r"$\tau" + " = {}".format(precession_time) + " ($\mathrm{\mu s}$)"
        )
...
if __name__ == "__main__":
    ...
    for filename in ["2022_11_22-16_23_02-johnson-search"]:
        ...
        did_we_initialize_each_rep = numpy.array(
            data["reinit_state_true_false_each_rep"]
        )
        num_reps = data["num_reps"]
        time_per_rep = numpy.zeros(num_reps)
        time_per_rep = (
            num_readouts_each_rep * (readout_time + 2000) / chop_factor
        ...
        if True:
            timestamp = tool_belt.get_time_stamp()
            save_path1 = tool_belt.get_file_path(
                __file__,
                timestamp,
                "-timetraces_"
                + "_{}".format(readout_dur)
                + "ms_{}".format(readout_power)
                + "mV_{}".format(init_time)
                + "ns",
            )
            save_path2 = tool_belt.get_file_path(
                __file__,
                timestamp,
                "-hists_"
                + "_{}".format(readout_dur)
            ...
            for ax in axes:
                width = nbins[nb]
                binned_data = (
                    sig_counts[: (sig_counts.size // width) * width]
                    .reshape(-1, width)
                    .sum(axis=1)
                )
                ts = (
                    numpy.linspace(
                        readout_dur, readout_dur * len(sig_counts), len(binned_data)
                    )
                    / 1000000
                )
                ax.plot(
                    ...
                    binned_data,
                    "r-",
                    label="{} ms ({}$\sigma$)".format(
                        averaging_times[nb] / 1e3, nb + 1
                    ),
                )
            ...
            axes[0].set_title(
                "readout = {} $\mu$s   power = {} V   init_time = {} ns".format(
                    readout_dur, readout_power / 1000, init_time
                )
            )
            plt.show()
