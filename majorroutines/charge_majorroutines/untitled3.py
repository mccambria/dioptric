    ...
    if do_fit:
        ...
        try:
            opti_params, cov_arr = curve_fit(
                tool_belt.gaussian, rad_dist, counts, p0=init_fit
            )
            if do_plot:
...
def build_xy_voltages_w_optimize(
    start_coords, CPG_coords, num_opti_steps, opti_scan_range
):
...
def data_collection_optimize(nv_sig, coords_list, run_num, opti_interval=4):
    with labrad.connect() as cxn:
        ret_vals = data_collection_optimize_with_cxn(
            cxn, nv_sig, coords_list, run_num, opti_interval
        )
...
def data_collection_optimize_with_cxn(
    cxn, nv_sig, coords_list, run_num, opti_interval=4
):
    """
    ...
    targeting.main_with_cxn(cxn, nv_sig, apd_indices)
    ...
    for i in range(num_samples):
        ...
        CPG_coord = [
            coords_list_drift[i][0],
            coords_list_drift[i][1],
...
def main_data_collection_with_cxn(cxn, nv_sig, coords_list):
    ...
    x_voltages, y_voltages = build_voltages_from_list(
        start_coords_drift, coords_list_drift
    )
...
def main(nv_sig, img_range, num_steps, num_runs, measurement_type, dz=0):
    ...
    if measurement_type == "1D":
        ...
        if type(img_range) == float:
            ...
            x_voltages = numpy.linspace(low_coords[0], high_coords[0], num_steps)
            y_voltages = numpy.linspace(low_coords[1], high_coords[1], num_steps)
            z_voltages = numpy.linspace(low_coords[2], high_coords[2], num_steps)
        ...
        title = "SPaCE - {} nm init pulse \n{} nm {} ms CPG pulse".format(
            init_color, pulse_color, pulse_time / 10**6
        )
    ...
    for n in range(num_runs):
        ...
        ret_vals = data_collection_optimize(
            nv_sig, coords_voltages_shuffle_list, n, opti_interval
        )
        ...
        if measurement_type == "2D":
            ...
            writePos = []
            readout_image_array = image_sample.populate_img_array(
                readout_counts_avg, readout_image_array, writePos
            )
...
if __name__ == "__main__":
    ...
    for file_name in file_list:
        ...
        ret_vals = plot_1D_SpaCE(
            file_name, path, do_plot=False, do_fit=True, do_save=False
        )
    ...
    lin_x_vals = numpy.linspace(x_vals[0], x_vals[-1], 100)
    # fig, ax = plt.subplots(1, 1, figsize=(10, 10))
    # ax.plot(x_vals, height_master_list, 'bo')
    ...
    init_fit = [13000]
    opti_params, cov_arr = curve_fit(
        inverse_sqrt, x_vals, widths_master_list, p0=init_fit
    )
    print("Opti params: " + str(opti_params))
    text = r"$A / t^{1/2}$"
    props = dict(boxstyle="round", facecolor="wheat", alpha=0.5)
