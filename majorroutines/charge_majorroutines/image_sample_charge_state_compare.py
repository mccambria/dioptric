...
def main_with_cxn(
    ...
    charge_readout_laser_power = tool_belt.set_laser_power(
        cxn, nv_sig, charge_readout_laser_key
    )
    ...
    xy_delay = tool_belt.get_registry_entry(
        cxn, "xy_small_response_delay", ["", "Config", "Positioning"]
    )
    # Get the scale in um per unit
    xy_scale = tool_belt.get_registry_entry(
    ...
    seq_args = [
        ...
        nvm_prep_laser_name,
        nv0_prep_laser_name,
        charge_readout_laser_name,
        nvm_prep_laser_power,
        nv0_prep_laser_power,
    ...
    ret_vals = cxn.pulse_streamer.stream_load(
        "charge_state_comparison.py", seq_args_string
    )
    period = ret_vals[0]
    ...
    x_voltages, y_voltages, x_voltages_1d, y_voltages_1d = build_xy_voltages(
        x_center, y_center, x_range, y_range, num_steps, num_reps
    )
    ...
    if plot_data:
        ...
        fig_nv0 = tool_belt.create_image_figure(
            img_array_nv0,
            img_extent,
            clickHandler=on_click_image,
    ...
    if plot_data:
        ...
        fig_nvm = tool_belt.create_image_figure(
            img_array_nvm,
            img_extent,
            clickHandler=on_click_image,
            color_bar_label="counts",
    ...
    if plot_data:
        ...
        title = r"Confocal scan, {}, {} us readout".format(
            charge_readout_laser_power, readout_us
        )
        fig = tool_belt.create_image_figure(
            img_array,
            img_extent,
            clickHandler=on_click_image,
            color_bar_label="Diff (counts)",
            title=title,
            um_scaled=um_scaled,
        )
...
if __name__ == "__main__":
    ...
    nv_sig = {
        ...
        "nvm_prep_laser": green_laser,
        "nvm_prep_laser_power": 6,
        "nvm_prep_laser_dur": 1e3,
        "nv0_prep_laser": red_laser,
        "nv0_prep_laser_value": 110,
