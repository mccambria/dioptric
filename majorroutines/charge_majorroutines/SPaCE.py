...
def plot_1D_SpaCE(
    file_name, file_path, do_plot=True, do_fit=False, do_save=True, scale=50000
):
    ...
    if do_plot:
        ...
        ax.set_title(
            "{} V, {} ms readout. with {} ms pulse".format(
                readout_pulse_power, readout_pulse_dur / 10**6, CPG_pulse_dur / 10**6
            )
        )
    ...
    if do_fit:
        ...
        try:
            opti_params, cov_arr = curve_fit(fit_func, rad_dist, counts, p0=init_fit)
            if do_plot:
                ...
                ax.plot(lin_radii, fit_func(lin_radii, *opti_params), "r-")
                text = (
                    "A={:.3f} sqrt(counts)\n$r_0$={:.3f} nm\n "
...
def collect_counts(
    cxn, num_movement_steps, num_samples, num_reps, seq_args_string, apd_indices
):
    """
    ...
    readout_counts = total_samples_list[
        rep_samples - 1 :: rep_samples
    ]  # readout pulse, which we want to keep these counts
...
def data_collection(nv_sig, opti_nv_sig, coords_list, apd_indices):
    with labrad.connect() as cxn:
        ret_vals = data_collection_with_cxn(
            cxn, nv_sig, opti_nv_sig, coords_list, apd_indices
        )
...
def data_collection_with_cxn(cxn, nv_sig, opti_nv_sig, coords_list, apd_indices):
    """
    ...
    opti_interval = tool_belt.get_registry_entry_no_cxn(
        "space_opti_interval_m", ["Config", "CommonDurations"]
    )  # in minutes
    ...
    xy_delay = tool_belt.get_registry_entry_no_cxn(
        "xy_large_response_delay", ["Config", "Positioning"]
    )
    ...
    # interval, then just run all the samples
        ...
        # Build the list to send to the piezo/galvo
        x_voltages, y_voltages, z_voltages = build_voltages_from_list_xyz(
            start_coords_drift, coords_list_drift, num_movement_steps, step_size_list
        )
...
def space_1D(
    ...
    x_voltages = numpy.linspace(start_coords[0], end_coords[0], num_steps)
    y_voltages = numpy.linspace(start_coords[1], end_coords[1], num_steps)
    z_voltages = numpy.linspace(start_coords[2], end_coords[2], num_steps)
    ...
    for n in range(num_runs):
        ...
        ret_vals = data_collection(
            nv_sig, opti_nv_sig, coords_voltages_shuffle_list, apd_indices
        )
...
def space_2D(
    ...
    ret_vals = build_voltages_2D_plot(
    ...
    c_voltages = numpy.linspace(
        start_coords[stationary_axis], start_coords[stationary_axis], len(a_voltages)
    )
