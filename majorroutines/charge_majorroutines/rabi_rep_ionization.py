...
def fit_data(uwave_time_range, num_steps, norm_avg_sig):
    ...
    try:
        popt, _ = curve_fit(fit_func, taus, norm_avg_sig, p0=init_params)
...
def create_fit_figure(
    uwave_time_range, uwave_freq, num_steps, norm_avg_sig, fit_func, popt
):
    min_uwave_time = uwave_time_range[0]
    max_uwave_time = uwave_time_range[1]
    taus = numpy.linspace(
...
def main(
    ...
    with labrad.connect() as cxn:
        rabi_per, sig_counts, ref_counts = main_with_cxn(
            cxn,
            nv_sig,
            apd_indices,
            uwave_time_range,
...
def main_with_cxn(
    ...
    if (fit_func is not None) and (popt is not None):
        fit_fig = create_fit_figure(
            uwave_time_range, test_uwave_freq, num_steps, norm_avg_sig, fit_func, popt
        )
