...
def main_with_cxn(cxn, nv_sig, apd_indices, num_reps, state):
    ...
    # Optimize
    opti_coords = targeting.main_with_cxn(cxn, nv_sig, apd_indices, 532, disable=True)
...
def optimize_num_ionizations(nv_sig):
    ...
    for num_ionizations in num_ion_list:
        ...
        sig_counts, ref_counts = main(nv_sig, apd_indices, num_reps, States.LOW)
    ...
    ax = axes[0]
    ax.plot(num_ion_list, sig_counts_avg, "ro", label="W/ pi-pulse")
    ax.plot(num_ion_list, ref_counts_avg, "ko", label="W/out pi-pulse")
    ax.set_xlabel("Number of repetitions")
    ax.set_ylabel("Counts (single shot measurement)")
...
def optimize_reion_and_init_ion_length(nv_sig):
    ...
    ax = axes[0]
    ax.plot(
        numpy.array(test_pulse_dur_list) / 10**3,
        sig_counts_avg,
        "ro",
        label="W/ pi-pulse",
    )
...
def optimize_shelf_pulse_length(nv_sig):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(17, 8.5))
    ax = axes[0]
    ax.plot(test_pulse_dur_list, sig_counts_avg, "ro", label="W/ pi-pulse")
    ax.plot(test_pulse_dur_list, ref_counts_avg, "ko", label="W/out pi-pulse")
...
def optimize_shelf_power(nv_sig):
    ...
    ax = axes[0]
    ax.plot(
        numpy.array(opt_power_list) * 10**3, sig_counts_avg, "ro", label="W/ pi-pulse"
    )
    ax.plot(
        numpy.array(opt_power_list) * 10**3,
        ref_counts_avg,
        "ko",
        label="W/out pi-pulse",
    )
    ...
    ax.set_title("Sweep shelf pulse power")
    text = (
        "Shelf pulse length = " + "%.0f" % (nv_sig["pulsed_shelf_dur"] / 10**3) + " us"
    )
    ax.text(
        0.55,
        0.90,
...
def optimize_yellow_pol_power(nv_sig):
    ...
    ax.set_title("Sweep yellow pol pulse power")
    text = (
        "Yellow pol. pulse length = "
        + "%.0f" % (nv_sig["yellow_pol_dur"] / 10**3)
        + " us"
    )
    ax.text(
        0.55,
...
if __name__ == "__main__":
    ...
    ensemble_B1 = {
        "coords": [-0.439, 1.4, 5.04],
        "name": "{}-B1".format(sample_name),
        "expected_count_rate": 6600,
        "nd_filter": "nd_0",
