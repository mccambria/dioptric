...
def plot_snr_v_dur(
    ...
    kpl.plot_points(ax, dur_list / 10**3, snr_list, color=KplColors.RED)
...
def determine_ionization_dur(nv_sig, num_reps, ion_durs=None):
    ...
    for ind in dur_ind_list:
        ...
        sig_counts, ref_counts = measure(nv_sig_copy, num_reps, state, plot=False)
        # print(sig_counts)
    ...
    max_snr = None
    try:
        fit_func = lambda x, coeff, mean, stdev: tool_belt.gaussian(
            x, coeff, mean, stdev, 0
        )
    ...
    title = "Sweep ionization pulse duration\n{} V {} ms readout".format(
        nv_sig["charge_readout_laser_power"], nv_sig["charge_readout_dur"] / 1e6
    )
    fig = plot_snr_v_dur(
        ion_durs,
        sig_counts_array,
        ref_counts_array,
        sig_counts_ste_array,
        ref_counts_ste_array,
        snr_array,
        title,
        fit_func=fit_func,
        popt=popt,
    ...
    file_path = tool_belt.get_file_path(
        __file__, timestamp, nv_sig["name"] + "-ion_pulse_dur"
    )
