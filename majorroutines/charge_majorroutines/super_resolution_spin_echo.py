...
def main(
    ...
    with labrad.connect() as cxn:
        sig_gate_counts, ref_gate_counts = main_with_cxn(
            cxn,
            nv_sig,
            opti_nv_sig,
            apd_indices,
...
def main_with_cxn(
    ...
    depletion_color = tool_belt.get_registry_entry_no_cxn(
    ...
    for run_ind in range(num_runs):
        ...
        for tau_ind in tau_ind_list:
            ...
            x_voltages, y_voltages = build_voltages(
                adjusted_nv_coords, adjusted_depletion_coords, num_reps
            )
...
# %%
def sweep_readout_time(
    nv_sig, opti_nv_sig, apd_indices, readout_time_list, depletion_coords, wait_time
):
    measurement_dur = 2
    num_runs = 1
    num_steps = 2
...
def sweep_readout_power(
    nv_sig, opti_nv_sig, apd_indices, readout_power_list, depletion_coords, wait_time
):
    measurement_dur = 2
    num_runs = 1
    num_steps = 2
    ...
    for p in readout_power_list:
        ...
        sig_counts_avg = numpy.average(sig_counts)
        ref_counts_avg = numpy.average(ref_counts)
        signal = numpy.average(
            numpy.array(sig_counts_avg) - numpy.array(ref_counts_avg)
        )
...
if __name__ == "__main__":
    ...
    nv_sig = {
        ...
        "spin_shelf_laser_filter": nd_yellow,
        "spin_shelf_laser_power": 0.4,
        "spin_shelf_dur": 0,
