...
def process_and_plot(data, rearrangement="spin_flip", file_path = None):
    ...
    sig_counts, ref_counts = threshold_counts(nv_list, sig_counts, ref_counts, method=thresh_method)
    ...
    sig_corr_coeffs = rescale_extreme_values(sig_corr_coeffs, sigma_threshold=0.3, method="tanh")
    ref_corr_coeffs = rescale_extreme_values(ref_corr_coeffs, sigma_threshold=0.3, method="tanh")
    ...
    if rearrangement == "spin_flip":
        ...
        nv_list, sig_corr_coeffs, ref_corr_coeffs = rearrange_by_corr_sign_individual(nv_list, sig_corr_coeffs, ref_corr_coeffs)
    elif rearrangement == "corr_sign":
        nv_list, sig_corr_coeffs, ref_corr_coeffs = rearrange_by_corr_sign(nv_list, sig_corr_coeffs, ref_corr_coeffs)
    else:
        pass
...
def plot_correlation_histogram(corr_matrix, bins=50):
    ...
    flattened_corr = corr_matrix[np.triu_indices_from(corr_matrix, k=1)]  # Only take upper triangle without diagonal
...
def draw_curved_edges(G, pos, ax, norm, edges, edge_colors, edge_widths, edge_alphas, curvature=0.2):
    """
...
def plot_nv_network(data):
    ...
    spin_up_nodes = [i for i, nv in enumerate(nv_list) if not nv.spin_flip]  # Spin-up: False (not flipped)
    spin_down_nodes = [i for i, nv in enumerate(nv_list) if nv.spin_flip]    # Spin-down: True (flipped)

    # Create the plot
    fig, ax = plt.subplots(figsize=(10, 8))

    # Draw spin-up nodes in red
    nx.draw_networkx_nodes(G, pos, nodelist=spin_up_nodes, node_color='red', node_size=60, label='Spin-up')
    
    # Draw spin-down nodes in blue
    nx.draw_networkx_nodes(G, pos, nodelist=spin_down_nodes, node_color='blue', node_size=60, label='Spin-down')
    ...
    threshold = 0.0  # Only draw edges if the correlation is above this absolute threshold
    ...
    draw_curved_edges(G, pos, ax, norm, edges, edge_colors, edge_widths, edge_alphas, curvature=0.6)
    ...
    plt.title("NV Center Network Graph with Curved Edges", fontsize=16)
