...
def calculate_metrics(nv_list, raw_data, method='otsu'):
    ...
    metrics = defaultdict(lambda: {"snr": [], "fidelity": [], "threshold": [], "probability_nv_minus": [], "probability_nv_zero": []})
    num_nvs = len(nv_list)
    ...
    for nv_ind in range(num_nvs):
        ...
        p_nv_minus = np.sum(ref_counts_list >= threshold) / len(ref_counts_list)  # Probability for NV- (|0>)
        p_nv_zero = np.sum(sig_counts_list >= threshold) / len(sig_counts_list)   # Probability for NV0 (|1>)
        ...
        if epsilon_0 == epsilon_1:
            snr = (2 * fidelity - 1) / np.sqrt(2 * fidelity * (1 - fidelity)) if fidelity < 1 else np.inf
        else:
            snr = (p_nv_minus - p_nv_zero) / np.sqrt(p_nv_minus * (1 - p_nv_minus) + p_nv_zero * (1 - p_nv_zero))
...
def visualize_metrics(metrics, datasets):
    ...
    probability_nv_minus = np.array([metrics[i]["probability_nv_minus"] for i in range(num_nvs)])
    probability_nv_zero = np.array([metrics[i]["probability_nv_zero"] for i in range(num_nvs)])
    ...
    plt.text(
        0.95, 0.4, r"$\text{SNR} = \frac{p_{0|0} - p_{0|1}}{\sqrt{p_{0|0}(1 - p_{0|0}) + p_{0|1}(1 - p_{0|1})}}$", 
    ...
    plt.text(
        0.95, 0.28, r"$\text{If } \epsilon_0 = \epsilon_1, \text{ SNR} = \frac{2\mathcal{F} - 1}{\sqrt{2\mathcal{F}(1 - \mathcal{F})}}$", 
        transform=plt.gca().transAxes, fontsize=12, ha="right", color="black", 
        bbox=dict(facecolor="white", edgecolor="grey", boxstyle="round,pad=0.3")
    )

    plt.text(
        0.95, 0.17, r"$\text{Fidelity } \mathcal{F} = 1 - \frac{1}{2} (\epsilon_0 + \epsilon_1)$", 
        transform=plt.gca().transAxes, fontsize=12, ha="right", color="black", 
        bbox=dict(facecolor="white", edgecolor="grey", boxstyle="round,pad=0.3")
    )

    plt.text(
        0.95, 0.07, r"$\text{where } \epsilon_0 = 1 - p_{0|0}, \; \epsilon_1 = p_{0|1}$", 
        transform=plt.gca().transAxes, fontsize=12, ha="right", color="black", 
        bbox=dict(facecolor="white", edgecolor="grey", boxstyle="round,pad=0.3")
    )
