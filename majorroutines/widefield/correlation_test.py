...
def create_raw_data_figure(nv_list, taus, counts, counts_ste):
    ...
    ax.set_xlabel("Random phase microwave pulse duration (ns)")
...
def create_correlation_figure(nv_list, taus, counts, norms):
    ### Make the figure

    # fig, ax = plt.subplots()
    fig, axes_pack = plt.subplots(
        nrows=5, ncols=5, sharex=True, sharey=True, figsize=[6.5, 6.0]
    )
