...
def rotation_matrix(axis, angle):
    ...
    if axis == "z":
        return numpy.array(
            [[numpy.exp(-1j * angle / 2), 0], [0, numpy.exp(1j * angle / 2)]]
        )
...
def simulate(drive_res, drive_rabi, nv_res, nv_rabi, num_pulses):
    ...
    for el in pulses_list:
        ...
        for nv_res in nv_res_distr:
            ...
            val = simulate_single(drive_res, drive_rabi, nv_res, nv_rabi, el)
            sum_pops += val
...
def main(
    nv_sig, apd_indices, state, max_num_pi_pulses, num_reps, num_runs, iq_delay=None
):
...
def main_with_cxn(
    ...
    if iq_delay is None:
        iq_delay = tool_belt.get_registry_entry(
            cxn, "iq_delay", ["", "Config", "Microwaves"]
        )
    ...
    norm_avg_sig_err = norm_avg_sig * numpy.sqrt(
        (err_sig_counts / avg_sig_counts) ** 2 + (err_ref_counts / avg_ref_counts) ** 2
    )
