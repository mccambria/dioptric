...
def main_with_cxn(
    ...
    if (xy_control_style == ControlStyle.STREAM) and (
        z_control_style == ControlStyle.STREAM
    ):
        xyz_control_style = ControlStyle.STREAM

    elif (xy_control_style == ControlStyle.STEP) or (
    ...
    z_delay = tool_belt.get_registry_entry(
        cxn, "z_delay", ["", "Config", "Positioning"]
    )
    z_scale = tool_belt.get_registry_entry(
    ...
    if xyz_control_style == ControlStyle.STEP:
        ...
        for i in range(total_num_samples):
            ...
            populate_img_array(
                [(actual_z_pos - cur_z_pos) * 1e3], dz_img_array, dz_img_write_pos
            )
