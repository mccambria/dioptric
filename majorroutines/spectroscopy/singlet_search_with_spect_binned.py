...
def main(
    ...
    month_path = f"G:/nvdata/pc_Nuclear/branch_master/singlet_search_with_spect_binned/{year}_{month}/{timestamp}"
        ...
        if validate_camera(_experiment):
            ...
            for run_ind in range(num_runs):
                ...
                for ind in tqdm(range(len(step_ind_list)), desc="Steps", position=0):
                    ...
                    wavelength_data = np.zeros((num_etalon, sensor_width))
                    ...
                    for eind in tqdm(
                        ...
                        total_counts_from_ROI = np.sum(image_data[1125:1145])
                        data_to_save[1, run_ind, num_steps_completed, eind] = (
                            total_counts_from_ROI
                        )

                        # Save to the NAS
                        wavelength_data[etalon_sett - min(etalon_settings)] = image_data
