...
def main(nv_sig, apd_indices, detuning,
         precession_time_range, num_steps, num_reps, num_runs, state = States.LOW):

    with labrad.connect() as cxn:
        main_with_cxn(cxn, nv_sig, apd_indices, detuning,
                      precession_time_range, num_steps, num_reps, num_runs, state)
...
#    save_tau_list = numpy.array([num_runs, len(taus)])
    ...
    seq_args = [min_precession_time, polarization_time, readout, uwave_pi_half_pulse, 0,
                max_precession_time, apd_indices[0], 
                laser_name, laser_power, state.value, state.value]
