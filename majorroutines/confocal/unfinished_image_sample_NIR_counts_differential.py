...
def main_with_cxn(
    ...
    if plot_data:
        ...
        fig = tool_belt.create_image_figure(
            ...
            img_extent,
            clickHandler=on_click_image,
            color_bar_label="Change in Counts (kcps)",
            title=title,
            um_scaled=um_scaled,
        )
    ...
    cxn.pulse_streamer.stream_start(
        total_num_samples * 2
    )  # CF: now we do two readouts at each pixel. need to alter how I am doing this.

    # timeout_duration = ((period*(10**-9)) * total_num_samples) + 10 # CF: removed
    timeout_duration = (
        (period * (10**-9)) * total_num_samples * 2
    ) + 10  # CF: now we do two readouts at each pixel
    ...
    while num_read_so_far < total_num_samples:
        ...
        if charge_initialization:
            ...
            time.sleep(1)  # CF: added for NIR
            new_samples_NIR = cxn.apd_tagger.read_counter_modulo_gates(
                2
            )  # CF: added for NIR
        ...
        if num_new_samples > 0:
            ...
            populate_img_array(
                new_samples_noNIR, img_array, img_write_pos
            )  # CF: for noNIR
