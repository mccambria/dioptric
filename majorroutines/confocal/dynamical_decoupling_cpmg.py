...
def create_fit_figure(
    ...
    smooth_taus = numpy.linspace(tau_T[0], tau_T[-1], num=1000)
...
def fit_t2_12C(data, fixed_offset=None, incremental=False):
    ...
    if fixed_offset:
        fit_func = lambda x, amp, decay: tool_belt.exp_stretch_decay(
            x, amp, decay, fixed_offset, 3
        )
        init_params = [-0.01, 1000]
    else:
        fit_func = lambda x, amp, decay, offset: tool_belt.exp_stretch_decay(
            x, amp, decay, offset, 3
        )
...
def main_with_cxn(
    ...
    for run_ind in range(num_runs):
        ...
        if do_plot:
            ...
            ax.cla()
            kpl.plot_line(
                ax, plot_taus, sig_counts_avg_kcps, color=KplColors.RED, label="signal"
            )
            kpl.plot_line(
                ax,
                plot_taus,
            ...
            kpl.plot_points(
                ax, plot_taus, norm_avg_sig, yerr=norm_avg_sig_ste, color=KplColors.BLUE
            )
            ...
            if do_scc:
                ax.set_title(
                    "CPMG-{} {} SCC Measurement".format(pi_pulse_reps, dq_text)
                )
            else:
    ...
    if do_plot:
        ...
        kpl.plot_line(
            ax, plot_taus, ref_counts_avg_kcps, color=KplColors.GREEN, label="reference"
        )
        ax.set_xlabel(r"Precession time, $T = 2 N \tau (\mathrm{\mu s}$)")
        ax.set_ylabel("kcps")
        ax.legend()
...
if __name__ == "__main__":
    ...
    for f in range(len(file_list)):
        ...
        kpl.plot_points(
            ax, tau_T, norm_avg_sig, yerr=norm_avg_sig_ste, label=label_list[f]
        )
        # ax.set_xscale('log')
        ax.set_xlabel(r"$T = 2 \tau$ ($\mathrm{\mu s}$)")
        ax.set_ylabel("Contrast (arb. units)")
    ...
    #                 ]
    file_list = [
        "2023_03_27-05_34_19-siena-nv0_2023_03_20",  # DQ CPMG-8
