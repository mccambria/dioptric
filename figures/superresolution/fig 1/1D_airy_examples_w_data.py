...
def eta_offset(r,v, P, t, offset, A, B, C):
    return A + B*numpy.exp(-v*t*intensity_airy_func_offset(r*C, P, offset)**2)
...
def do_plot(file, folder, scale):
    ...
    eta_n = lambda r, t, offset, A, B, C: eta_offset(r,v, P, t, offset, A, B, C)
    ...
    #       )
    # print(opti_params)
    opti_params = [326.13302951,   4.22767238,   0,    3.06717115 ,  0.74011654]
    fig, ax = plt.subplots()
    fig.set_figwidth(fig_w)
    fig.set_figheight(fig_l)

    
    ax.set_xticks([0, 300/opti_params[4]])
    ax.set_yticks([])
    ax.set_ylim([-0.000002, 0.00002])

    # x_lin = numpy.linspace(-1500, 1500, 601)
    x_lin = numpy.linspace(-600, 1500, 601)
    y_vals = intensity_airy_func_offset(x_lin*opti_params[4], P, opti_params[1]) 
    ...
    x_lin = numpy.linspace(-1300, 1300, 601)
    y_vals = intensity_airy_func_offset(x_lin*opti_params[4], P, 0)
    ax.plot(x_lin, y_vals, '-', color= color_red,  linewidth = 1)
    y_vals = eta_n(x_lin, *opti_params)
