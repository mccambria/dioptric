...
def plot_1D_SpaCE(
    ...
    if do_fit:
        ...
        try:
            opti_params, cov_arr = curve_fit(
                fit_func, rad_dist, counts, p0=init_fit
            )
            ...
            if do_plot:
                ...
                ax.plot(
                    lin_radii,
                    fit_func(lin_radii, *opti_params),
                    "r-",
                    linestyle="solid",
                    linewidth=1,
                )
    ...
    if do_plot and do_save:
        filePath = tool_belt.get_file_path("SPaCE", timestamp, nv_sig["name"])
...
def plot_width_vs_dur(file_list, path, threshold, e):
    ...
    for f in range(len(file_list)):
        ...
        ret_vals = plot_1D_SpaCE(
            file_name, path, threshold, do_plot=False, do_fit=True
        )
    ...
    widths_master_list_x = (
        2 * numpy.pi * NA * widths_master_list_fwhm / wavelength
    )
    ...
    print(
        "alpha = {:.14f} +/- {:.14f}".format(
            opti_params[0], numpy.sqrt(cov_arr[0][0])
        )
    )
...
def plot_heights_vs_dur(file_list, path, threshold, Alpha):
    ...
    print(
        "A: " + str(opti_params[0]) + " +/-" + str(numpy.sqrt(cov_arr[0][0]))
    )
    print(
        "epsilon: "
        + str(opti_params[1])
        + " +/- "
        + str(numpy.sqrt(cov_arr[1][1]))
    )
