# -*- coding: utf-8 -*-
"""
Created on Wed Jan 26 15:11:51 2022
...
datadir = 'E:/Shared drives/Kolkowitz Lab Group/nvdata/pc_rabi/branch_master/SPaCE/2021_10'
...
def canny_edge_detector(readout_image_array,num_steps):
    ...
    keep = filter_strong_edges(gradient, width, height, 0.15, 0.23)
    #print (keep)
...
def find_centers(file, do_plot = False):
    ...
    if do_plot:
        ...
        img2 =ax2.imshow(empty_array, extent = extent, cmap = 'inferno')
    ...
    rmax = 26#25
    steps =91
    threshold = 0.2# percentage of proper pixels on the circle
    ...
    for k, v in sorted(acc.items(), key=lambda i: -i[1]):
        x_pos, y_pos, r = k
        #print (v/steps)
        if v / steps >= threshold and all( abs(r - rc) > 1 for xc, yc, rc in circles): #and all((x_pos - xc) ** 2 + (y_pos - yc) ** 2 > rc ** 2 for xc, yc, rc in circles):
    ...
    c_x = x[circles[0][1]]
    c_y = y[circles[0][0]]
    c_r = circles[0][2]*full_pix 
    
    d_x = x[circles[1][1]]
    d_y = y[circles[1][0]]
    d_r = circles[1][2]*full_pix 
    
    r_diff = numpy.sqrt((c_x - d_x)**2 + (c_y - d_y)**2)
    print('radial dist between points = {} V'.format(r_diff))
    
    return c_x, c_y, c_r, d_x, d_y, d_r


if __name__ == "__main__":
    file = '2021_10_17-19_02_22-johnson-dnv5_2021_09_23.txt'
