...
def fit_resonance(
    ...
    fit_func = lambda freqs, a1, s1, a2, s2, a3, s3: tri_gaus(freqs,freq_center, hyperfine, a1, s1, a2, s2, a3, s3)
...
def do_plot(rabi_file, pesr_file_list, act_freq_center):
    ...
    kpl.plot_points(
        ax, taus, norm_avg_sig,  color=KplColors.BLUE
    )
    # ax.plot(taus, norm_avg_sig,  '.', color = KplColors.BLUE)
    kpl.plot_line(
        ax, smooth_taus, fit_func(smooth_taus,*popt), color=KplColors.RED
    )
    ...
    for file in pesr_file_list:
        ...
        kpl.plot_line(
            ax, smooth_freqs_MHz, fit_func(smooth_freqs,*popt) + ni, color=KplColors.RED
        )
