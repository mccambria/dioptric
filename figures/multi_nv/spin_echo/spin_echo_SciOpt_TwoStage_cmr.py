...
def fit(total_evolution_times, nv_counts, nv_counts_ste):
    ...
    ### Replace the original brute force section with two-stage approach
    t0 = time.time()
    ...
    timing['optimization_setup'] = time.time() - t0

    t0 = time.time()
    # Stage 1: Coarse grid search
    ranges = [(0, 1.5), (0, 0.5)]
    workers = multiprocessing.cpu_count() - 1  # Optimize worker count
    rough_minimum = brute(
        brute_fit_fn_cost, 
        ranges, 
        Ns=50,  # Coarser grid for initial search
        workers=workers, 
        args=args,
        finish=None
    )
    timing['coarse_search'] = time.time() - t0

    t0 = time.time()
    # Stage 2: Refine with basin-hopping
    refined_result = basinhopping(
        brute_fit_fn_cost,
        x0=rough_minimum,
        niter=100,
        T=1.0,
    ...
    popt = refined_result.x  # Get the optimized parameters
    ...
    # Add new timing information to log
    timing['total'] = time.time() - fit_start
    
    # Update the log file writing section to include new timing information
    ...
    with open(fit_log_filename, 'w') as log_file:
        ...
        log_file.write(f"\nOptimization setup: {timing['optimization_setup']:.3f} seconds")
        log_file.write(f"\nCoarse grid search: {timing['coarse_search']:.3f} seconds")
        log_file.write(f"\nRefinement step: {timing['refinement']:.3f} seconds")
