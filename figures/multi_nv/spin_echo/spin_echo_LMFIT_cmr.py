...
def quartic_decay(
    ...
    exp_part = np.exp(
        -(((tau_expanded - revival_times_expanded) / quartic_decay_time) ** 4)
    )
...
def fit(total_evolution_times, nv_counts, nv_counts_ste):
    ...
    full_bounds = [[0, 0, 0, 0, 0, -0.5, 0, 0], [1, 1, 20, 1000, 10, 0.5, 1.5, 0.5]]
...
def create_interactive_fit_figures(data, nv_inds=None):
    ...
    def navigate_figures():
        ...
        print(
            f"Currently showing: NV {nv_inds[current]} (Figure {current + 1}/{len(nv_inds)})"
        )
        ...
        while True:
            ...
            update_plot(current)
            print(
                f"Showing: NV {nv_inds[current]} (Figure {current + 1}/{len(nv_inds)})"
            )
...
def create_fit_figure(data, axes_pack=None, layout=None, no_legend=True, nv_inds=None):
    ...
    if do_fit:
        ...
        for i, nv_ind in enumerate(nv_inds):
            nv_timing = {
                "data_prep": 0,
                "fitting": {"function_call": 0, "chi_sq_calc": 0},
                "plotting": {"points": 0, "line": 0, "formatting": 0, "drawing": 0},
                "total": 0,
            }
            nv_start = time.time()
    ...
    for ind in range(2):
        summary_timing = {"setup": 0, "plot_fit": 0, "axis_formatting": 0, "total": 0}
    ...
    with open(log_filename, "w") as log_file:
        ...
        for i, nv_timing in enumerate(timing["fitting"]["per_nv"]):
            log_file.write(f"\n  NV {nv_inds[i]}:\n")
            log_file.write(
                f"    Data preparation: {nv_timing['data_prep']:.3f} seconds\n"
            )
            log_file.write(f"    Fitting:\n")
            log_file.write(
                f"      Function call: {nv_timing['fitting']['function_call']:.3f} seconds\n"
            )
            log_file.write(
                f"      Chi square calculation: {nv_timing['fitting']['chi_sq_calc']:.3f} seconds\n"
            )
            log_file.write(f"    Plotting:\n")
            log_file.write(
                f"      Points: {nv_timing['plotting']['points']:.3f} seconds\n"
            )
            log_file.write(f"      Line: {nv_timing['plotting']['line']:.3f} seconds\n")
            log_file.write(
                f"      Formatting: {nv_timing['plotting']['formatting']:.3f} seconds\n"
            )
            log_file.write(
                f"      Drawing: {nv_timing['plotting']['drawing']:.3f} seconds\n"
            )
        ...
        for i, summary_timing in enumerate(timing["plotting"]["summary_figures"]):
            ...
            log_file.write(
                f"    Axis formatting: {summary_timing['axis_formatting']:.3f} seconds\n"
            )
