...
def plot_count_rate(angles, count_rate, title, do_fit = True):
    ...
    if do_fit:
        fit_func = lambda t,  amp, offset, freq, phase: tool_belt.sin_phase(t, amp, offset, freq, phase)
    ...
    if do_fit:
        kpl.plot_line(ax, angles_lin, fit_func(angles_lin_rad,*popt ), label = 'fit', color=KplColors.RED)
