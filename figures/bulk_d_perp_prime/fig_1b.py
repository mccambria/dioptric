...
def main(folder, file_high, file_zero, file_high_to_low,
    ...
    source = 't1_double_quantum'
    folder = 'paper_data/bulk_dq/' + folder

    # Get reference values for to convert fluorescence to population
    ref_range = [None, None]

    # Reference for 0
    data = tool_belt.get_raw_data(source, file_high_to_low, folder)
    ref_range[0] = get_first_norm_avg_sig(data)

    # Reference for 1
    data = tool_belt.get_raw_data(source, file_zero, folder)
    ref_range[1] = get_first_norm_avg_sig(data)

    # %% Relaxation out of zero

    raw_data_zero = tool_belt.get_raw_data(source, file_zero, folder)
    ...
    # %% Relaxation out of high

    raw_data_high = tool_belt.get_raw_data(source, file_high, folder)
