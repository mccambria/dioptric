...
def main(
    ...
    # Reference for 0
    data = tool_belt.get_raw_data(file_high_to_low, path)
    # ref_range[0] = get_first_norm_avg_sig(data)
    ref_range[0] = 0.64

    # Reference for 1
    data = tool_belt.get_raw_data(file_zero, path)
    ref_range[1] = get_first_norm_avg_sig(data)
    # print(ref_range)
    # return

    raw_data_zero = tool_belt.get_raw_data(file_zero, path)
    signal_zero, ste_zero, times_zero = process_raw_data(
        raw_data_zero, ref_range
    )
    smooth_t = numpy.linspace(times[0], times[-1], 1000)
    fit_zero = relaxation_zero_func(
        smooth_t, gamma, omega, pi_pulse_infidelity
    )

    raw_data_high = tool_belt.get_raw_data(file_high, path)
    signal_high, ste_high, times_high = process_raw_data(
        raw_data_high, ref_range
    )
    smooth_t = numpy.linspace(times[0], times[-1], 1000)
    fit_high = relaxation_high_func(
        smooth_t, gamma, omega, pi_pulse_infidelity
    )
