...
def subtraction_plot(ax, analysis_file_path):
    ...
    ax.scatter(x_clip, y_clip,
               zorder=5, marker='o',
               color='#CC99CC', edgecolor='#993399', s=ms**2)
...
def main(folder, file_high, file_zero, file_high_to_low,
    ...
    ax.plot(smooth_t, fit_zero, color='#56B4E9', linewidth=lw)
    try:
    ...
    ax.scatter(times_zero[:times_15], signal_zero[:times_15],
               zorder=5, marker='s',
               color='#56B4E9', edgecolor='#0D83C5', s=ms**2)

    # Plot high
    high_label = 'Relaxation \nout of {}'.format(r'$\ket{+1}$')
    ax.plot(smooth_t, fit_high, color='#F0E442', linewidth=lw)
    try:
    ...
    ax.scatter(times_high[:times_15], signal_high[:times_15],
               zorder=5, marker='D',
               color='#F0E442', edgecolor='#D2C40E', s=ms**2)
    
    zero_patch = mlines.Line2D([], [], label=zero_label, linewidth=lw,
                               marker='s', color='#56B4E9',
                               markeredgecolor='#0D83C5', markersize=ms)
    high_patch = mlines.Line2D([], [], label=high_label, linewidth=lw,
                               marker='D', color='#F0E442',
                               markeredgecolor='#D2C40E', markersize=ms)
