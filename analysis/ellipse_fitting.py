...
def main_sub(ellipse, do_plot=False):
    ...
    res = minimize(
        corr_cost, (cov_phi,), args=(points,), bounds=((0, np.pi / 2),)
    )
    opti_phi = res.x[0]
    # opti_phi = true_phi
    # opti_phi = cov_phi
    ...
    if do_plot:
        ...
        phi_lin = np.linspace(0, 0.02, 100)
        corr_cost_lambda = lambda phi: corr_cost(phi, points)
        with Pool() as p:
            costs = p.map(corr_cost_lambda, phi_lin)
        kpl.plot_line(ax, phi_lin, costs)
        ...
        #         100 * point[1],
        #         size="tiny",
        #         color=KplColors.RED.value,
        #     )

        plt.show(block=True)

    # if abs(opti_phi - true_phi) > 0.1:
    #     print("bad ind")
...
def main(path):
    ...
    if parallel:
        main_sub_lambda = lambda ellipse: main_sub(ellipse)
        with Pool() as p:
            algo_phis = p.map(main_sub_lambda, ellipses)

    fig, ax = plt.subplots()
    kpl.plot_points(ax, true_phis, algo_phis)
    kpl.tight_layout(fig)

    phi_errors = [ela - elb for ela, elb in zip(algo_phis, true_phis)]
...
if __name__ == "__main__":
    ...
    ml_errs_old = [-0.00046665140253709936, 0.009474464047654285, 0.001201024195885636, -0.010188893219576556, -0.006293576703638365, 0.00528818813059434, -0.002156792868103219, -0.008029065500972099, -0.008315859612441745, 0.01784627704619024, 0.005192718653231898, 0.01199185765884564, 0.0019663950024604304, -0.0054209627781149905, 0.0016656947481628048, -0.020406945949929867, -0.01639897576990812, -0.010088480286173929, 0.007648381692518302, -0.009738136048462243, -0.01577058139171239, -0.008144181538362438, 0.009144270972394564, 0.008150482674917747, -0.006457179685570136, 0.013889627131220217, 0.0031201731979633163, 0.0047703528676938944, -0.0030556744036221994, -0.006693699082918725, 0.01706155864435277, -0.0058137497470840405, 0.0030777020416703493, 0.0038705737704335386, 0.0025323862241084516, -0.004199238803306804, 0.003990157572617337, -0.0030918318170826686, 0.0006207849945250032, -0.0008094085852986543, 0.0017879409481820208, -0.0002935435872182257, -0.0043252775409602096, 0.002689359176464845, -0.004393849663061666, -0.004973362815869714, -0.018786357870603787, -0.0020026897801433408, 0.00913010763969846, -0.009565111596656628, 0.0015455407788378395, -0.008466692703044565, 0.004862890652706731, 0.005202623822421426, -0.0006155489055915953, 0.006066151977095258, 0.0019267262939968166, 0.009789991824032285, 0.012197345761154832, -0.011972715487737137, 0.012471269376532979, 0.0008374099719967998, -0.0008030408082980722, 0.013161007366271349, -0.005448018532702603, -0.006630080967073737, 0.006255294808707124, 0.006326326694090889, 0.007657138726433921, -0.009783068297701925, 0.00396149420535763, 0.016535539690713308, -0.00267087749382644, -0.006125132479587014, 0.006282191828616668, -0.001299203503899865, -0.0031602466274196495, -0.008720253397070943, -0.004293781380029227, 0.006401272166759953, -0.0029456641127958427, -0.008307344756602164, -0.006173048773110774, 0.014364392665063841, 0.004795574679308912, -0.0064262542751553076, 0.0017156873873147571, -0.015254171183672893, 0.0066381224985546106, 0.0037647883098059776, 0.007453366030898634, 0.006408981057732133, 0.003136887072351102, 0.000237030099369262, -0.006593678507863752, 0.0028091472605895618, 0.010048432247072858, 0.012007443701801068, -8.507579394234632e-05, 0.01189965205220056]
    ml_errs = [-0.0012240674232477655, 0.006982200220468471, -0.00138381021156464, -0.010250507227445416, -0.0046714186469833385, 0.004428472890517454, -9.277523346673533e-05, -0.007037603208740584, -0.007011164865807329, 0.01811043142387897, 0.006090471955624066, 0.00973587861697095, 6.32421124338034e-05, -0.00640277826415625, 0.0028884797364990833, -0.020496313323547177, -0.017190587557380343, -0.008960967889319438, 0.009408482804396101, -0.009122286040976368, -0.013337153954502634, -0.00727099513225965, 0.01040763506323783, 0.006061297499369789, -0.005251015924565561, 0.014570101373501565, 0.0012743810507325204, 0.0029514762417574836, -0.0011945725458506473, -0.005449976278516955, 0.014582619400582697, -0.006489193460154419, 0.0044405856724252235, 0.0024850593021275014, 0.0025323862241084516, -0.004881711860181631, 0.003977671139520189, -0.00511197624366988, 0.002598665425514346, -0.002005695851464151, 0.003981138375795723, -0.0024754286907506806, -0.006864826617182088, 0.0007434989366283951, -0.006508037240880782, -0.006253726325271242, -0.019018508287211855, 0.0005393306468803039, 0.00881055677328546, -0.008597988705740356, 0.0012242879254034111, -0.009363326653436754, 0.005500479940221847, 0.005085508485299095, -0.00258093542620369, 0.006880740099117988, 0.003089703414050904, 0.009753975916705793, 0.014634856672516294, -0.01077837609225514, 0.013444128464508154, -0.0003842519649719378, -0.0013846720157720815, 0.013381208307718273, -0.005492408835995111, -0.0064489848773479075, 0.003697207175988426, 0.004335365828488036, 0.007548564487559917, -0.011042451453170177, 0.004594279200801243, 0.01851960999177238, -0.00183898475552291, -0.006026857294674587, 0.007086385235663517, 0.0010787848383382592, -0.002733740348760394, -0.009637276098479752, -0.0023944161980979928, 0.005276794721304989, -0.0007421864418812874, -0.007417453780096783, -0.005319681866758359, 0.013049487157728357, 0.0032714344836721754, -0.008603460139511299, 0.001514554755512404, -0.014003458194852714, 0.005932177883695711, 0.0031078058610398696, 0.00737077193027269, 0.007111770772916415, 0.004914016365875684, 0.0007634285455883072, -0.01184717792858247, 0.0005886400424423011, 0.010125052982117522, 0.011503660277001004, -0.001071090120835727, 0.011408240113458312]
