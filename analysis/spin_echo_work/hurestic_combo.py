...
def plot_sorted_feasible(freqs_kHz, title="Sorted feasible ESEEM lines", f_range_kHz=(150,20000)):
    """Your 'index vs frequency' discrete plot (Y on log scale)."""
    if freqs_kHz is None or len(freqs_kHz) == 0:
        print("No frequencies to plot."); return
    f = np.asarray(freqs_kHz, float)
    idx = np.arange(1, f.size+1)
    fig, ax = plt.subplots(figsize=(7.5, 5.0))
    ax.plot(idx, f, ".", ms=2.5)
    ax.set_yscale("log")
...
# ===== Usage example (drop-in) ===============================================
recs = select_records(load_catalog("analysis/spin_echo_work/essem_freq_catalog_22A.json"),
                      fmin_kHz=150, fmax_kHz=20000, orientations=((1,1,1),))

# A) Fundamentals only (what you usually see) — discrete, experiment-matching set
# packA = build_feasible_freqs_from_recs(
#     recs, p_occ=0.011, f_range_kHz=(10, 20000),
#     include_combos=False, min_sep_kHz=1.0, top_M_fundamentals=80, weight_floor=0.0
# )
# plot_sorted_feasible(packA["fund_freq_kHz"], title="Fundamentals (sorted)")

# B) (Optional) Add pairwise sums/differences (tiny lines, only if needed)
packB = build_feasible_freqs_from_recs(
    recs, p_occ=0.011, f_range_kHz=(0.1, 25000),
    include_combos=True, min_sep_kHz=1.0, top_M_fundamentals=130, weight_floor=0.0
)
plot_sorted_feasible(packB["all_freq_kHz"], title="Fundamentals + pairwise combos (sorted)")

# Feed into your fitter (cycles/µs band must cover these):
# gridA = build_override_grid_for_fitter(packA["fund_freq_kHz"], allow_single=True)
# # Example fit call:
...
#     freq_seed_n_peaks=6,
#     extra_overrides_grid=gridA,      # <<— restrict sweep to physics-feasible sticks
