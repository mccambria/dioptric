...
def generate_fake_data(theta_B, phi, par_E, perp_E):

    fit_vec = [theta_B, phi, par_E, perp_E]
...
def calc_single_hamiltonian(mag_B, theta_B, phi, par_E, perp_E):
...
def calc_hamiltonian(mag_B, theta_B, phi, par_E, perp_E):
    if (type(mag_B) is list) or (type(mag_B) is numpy.ndarray):
        ...
        return calc_single_hamiltonian(mag_B, theta_B, phi, par_E, perp_E)


def calc_res_pair(mag_B, theta_B, phi, par_E, perp_E):
    hamiltonian = calc_hamiltonian(mag_B, theta_B, phi, par_E, perp_E)
    if (type(mag_B) is list) or (type(mag_B) is numpy.ndarray):
...
def find_mag_B(res_desc, theta_B, phi, par_E, perp_E):
    ...
    args = (res_desc, theta_B, phi, par_E, perp_E)
...
def find_mag_B_objective(x, res_desc, theta_B, phi, par_E, perp_E):
    calculated_res_pair = calc_res_pair(x, theta_B, phi, par_E, perp_E)
...
def plot_resonances(mag_B_range, theta_B, phi, par_E, perp_E,
                    name='untitled'):

    smooth_mag_B = numpy.linspace(mag_B_range[0], mag_B_range[1], 1000)
    res_pairs = calc_res_pair(smooth_mag_B, theta_B, phi, par_E, perp_E)
...
def chisq_func_known_E(fit_vec_known_E, par_E, perp_E, res_descs):

    fit_vec = list(fit_vec_known_E)
    fit_vec.extend([par_E, perp_E])
    return chisq_func(fit_vec, res_descs)
...
def main(name, res_descs):
    ...
    # fit_vec = [theta_B, phi, par_E, perp_E]
    param_bounds = ((0, pi/2), (0, 2*pi/3), (-0.050, 0.050), (0, 0.050))
    ...
    ############ Guess par_E and perp_E by zero field ############
    ...
    if (par_E is not None) and (perp_E is not None):
        # Just fit for theta_B and phi
        param_ranges = param_bounds[0:2]
        args = (par_E, perp_E, res_descs)
        x0 = brute(chisq_func_known_E, param_ranges, args=args, Ns=20)
        guess_params = list(x0)
        guess_params.extend([par_E, perp_E])
    else:
...
if __name__ == '__main__':
    ...
    # fit_vec = [0.0, 0], 0.010, 0
    # name = 'test2'
    # res_descs = [[0.0, 2.88, 2.88], 
    #             [None, 2.86, 2.9],
    #             [None, 2.82, 2.94], 
    #             [None, 2.78, 2.98], 
    #             [None, 2.63, 3.13], 
    ...
    # fit_vec = [pi/3, pi/4, 0.010, 0.025]
    # name = 'test3'
    # res_descs = [[0.0, 2.855, 2.905], 
    #             [None, 2.853, 2.907],
    #             [None, 2.843, 2.92], 
    #             [None, 2.828, 2.939], 
    #             [None, 2.778, 3.031], 
    ...
    # fit_vec = [2*pi/5, 2*pi/3, -0.017, 0.019]
    name = 'test4'
    ...
    # fit_vec = [1.020, 2*pi/3, 0.050, 0.050]
    name = 'test5'
    res_descs = [[0.0, 2.87, 2.97],
                [None, 2.869, 2.971],
                [None, 2.862, 2.981], 
                [None, 2.851, 2.996], 
                [None, 2.802, 3.084], 
    ...
    # Test plot
    # args: mag_B_range, theta_B, phi, par_E, perp_E
    # plot_resonances([0, 0.5], pi/2, 0, 0, 0)

    # Fake data
    # args: theta_B, phi, par_E, perp_E
    # bounds: ((0, pi/2), (0, 2*pi/3), (-0.050, 0.050), (0, 0.050))
    # generate_fake_data(1.242, 0.000, -0.017, 0.019)
