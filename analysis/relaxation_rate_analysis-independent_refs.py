...
def get_data_lists(folder_name):
    ...
    for file in file_list:
        ...
        try:
            ...
            norm_avg_sig_independent_refs = sig_counts[:num_runs] / ref_counts[:num_runs]
            norm_avg_sig = numpy.average(norm_avg_sig_independent_refs[:num_runs], axis=0)
            norm_avg_sig_ste = numpy.std(norm_avg_sig_independent_refs[:num_runs], axis=0, ddof = 1) / numpy.sqrt(num_runs)
...
#                    'gamma_opti_params': gamma_opti_params.tolist(),
        ...
        file_name = '{}-analysis-independent_refs'.format(folder)
