# -*- coding: utf-8 -*-
"""Calculate the estimated time for a T1 experiment, including how often we 
optimize, and calculate an expected SNR
...
def expected_st_dev_norm(ref_counts, expected_contrast):
    ...
    rel_std_sig = np.sqrt(sig_counts) / sig_counts
    rel_std_ref = np.sqrt(ref_counts) / ref_counts
    # Propogate the error
    error = (1 - expected_contrast) * np.sqrt(
        (rel_std_sig ** 2) + (rel_std_ref ** 2)
    )
...
def t1_exp_times(
    exp_array, contrast, exp_count_rate, readout_window, overhead
):
    total_exp_time_list = []
    ...
    for line in exp_array:
        ...
        exp_time_s = (
            sequence_time * num_steps + optimize_time
        ) * num_runs  # seconds
        ...
        print(
            "{}: {} hours, optimize every {} minutes, expected snr: {}".format(
                exp, "%.1f" % exp_time_h, "%.2f" % opti_time, "%.1f" % snr
            )
        )
